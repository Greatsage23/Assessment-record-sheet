...
export default function QuestionBank() {
  ...
  const builtInQuestions = useMemo(() => scope.subject === "Computing" ? buildComputingQuestionBank(scope.className) : [], [scope.className, scope.subject]);
  const allQuestions = useMemo(() => [...builtInQuestions, ...questions], [builtInQuestions, questions]);
  const topics = useMemo(() => [...new Set(allQuestions.map((question) => question.topic))].sort(), [allQuestions]);
  useEffect(() => {
    if (scope.subject === "Computing") {
      const curriculumTopics = computingTopicsForClass(scope.className);
      setFilters((current) => curriculumTopics.includes(current.topic) ? current : { ...current, topic: curriculumTopics[0] });
      setForm((current) => curriculumTopics.includes(current.topic) ? current : { ...current, topic: curriculumTopics[0] });
    }
  }, [scope.className, scope.subject]);
  const visible = useMemo(() => allQuestions.filter((question) => {
  ...
  async function deleteQuestion(question: Question) {
    if (question.source === "Built-in") {
      setNotice("Built-in curriculum questions are protected. Select them for a paper or add your own alternative question.");
      return;
    }
  ...
  return <section className="question-workspace">
    <header className="question-hero"><div><p>Free teaching tool</p><h2>Question Bank &amp; Paper Generator</h2><span>Use the complete built-in Computing curriculum bank or add questions created by your teachers—no AI fees.</span></div><strong>{allQuestions.length}<small>bank questions</small></strong></header>
    ...
    {scope.subject === "Computing" && <div className="computing-bank-tabs" aria-label="Computing question type"><button className={filters.questionType === "All" ? "active" : ""} onClick={() => setFilters({ ...filters, questionType: "All" })}>All 65</button><button className={filters.questionType === "Objective" ? "active" : ""} onClick={() => setFilters({ ...filters, questionType: "Objective" })}>50 Objective</button><button className={filters.questionType === "Theory" ? "active" : ""} onClick={() => setFilters({ ...filters, questionType: "Theory" })}>10 Theory</button><button className={filters.questionType === "Practical" ? "active" : ""} onClick={() => setFilters({ ...filters, questionType: "Practical" })}>5 Practical</button></div>}
    {scope.subject === "Computing" && <button className="curriculum-full-bank" onClick={() => { setPaper(visible); setNotice(`${visible.length} curriculum questions added to the paper.`); }}>Use all {visible.length} questions in the current topic and section</button>}
