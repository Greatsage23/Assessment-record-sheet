...
export default function QuestionBank() {
  const [scope, setScope] = useState({ className: "Basic 8 Red", subject: "Mathematics", term: "Term 1" });
  ...
  async function addQuestion(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/questions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create", ...scope, ...form, options: form.optionsText.split("\n") }) });
  ...
  return <section className="question-workspace">
    ...
    <section className="question-scope panel"><div><span>Question scope</span><small>Changing these selections loads the matching bank.</small></div><label>Class<select value={scope.className} onChange={(event) => setScope({ ...scope, className: event.target.value })}>{CLASSES.map((item) => <option key={item}>{item}</option>)}</select></label><label>Subject<select value={scope.subject} onChange={(event) => setScope({ ...scope, subject: event.target.value })}>{SUBJECTS.map((item) => <option key={item}>{item}</option>)}</select></label><label>Term<select value={scope.term} onChange={(event) => setScope({ ...scope, term: event.target.value })}>{TERMS.map((item) => <option key={item}>{item}</option>)}</select></label><button className="primary compact" onClick={() => setShowAdd((value) => !value)}>{showAdd ? "Close form" : "+ Add question"}</button></section>
