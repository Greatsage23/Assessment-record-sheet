...
const defaultPaperType = (subject: string, topic: string): PaperType => subject === "English Language" && englishShortAnswerTopics.has(topic) ? "Short Answer" : subject === "English Language" && englishEssayTopics.has(topic) ? "Essay" : "Objective";
...
export default function QuestionBank() {
  ...
  async function addQuestion(event: FormEvent) {
    ...
    try {
      ...
      if (isLiterature && !form.literatureBook.trim()) throw new Error("Enter the literature book or text title.");
      const storedTopic = isLiterature ? `${form.topic}: ${form.literatureBook.trim()}${form.literatureAuthor.trim() ? ` by ${form.literatureAuthor.trim()}` : ""}` : form.topic;
  ...
  return <section className="question-workspace">
    ...
    <section className="panel topic-planner"><div className="panel-head"><div><h2>3. Select topics and paper sections</h2><p>{scope.subject === "Computing" ? "Assign topics to Objective, Essay or Practical. The system randomly chooses 40 MCQs, four theory questions and one practical." : "Select each topic, question type and required quantity."}</p></div><button className="secondary compact" onClick={() => setShowAdd((value) => !value)}>{showAdd ? "Close form" : "+ Add question"}</button></div>
      {scope.subject === "English Language" && <div className="literature-outline"><div><strong>English Language aspects</strong><span>Oral language · Reading · Grammar · Vocabulary · Writing</span></div><div><strong>Separate Literature section</strong><span>Prose · Drama · Poetry · Oral literature</span><small>For each literature question, identify the exact book or text, author, plot or subject matter, characters, themes, setting, language/style and lessons or values learners are expected to study.</small></div></div>}
