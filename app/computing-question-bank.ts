...
function practicalQuestions(className: string, pack: TopicPack): BuiltInQuestion[] {
  return Array.from({ length: 5 }, (_, index) => {
    ...
    const answer = `PRACTICAL ASSESSMENT RUBRIC — 20 MARKS\nAccuracy and correct application of ${concept.term}: 6 marks.\nLogical procedure and completion of required steps: 4 marks.\nFunctional or technically valid output: 4 marks.\nPresentation, clarity and appropriate design: 3 marks.\nCorrect filename, location and file management: 3 marks.\nAccept an equivalent paper simulation where computer access is limited.`;
    return { id: stableId(`${className}-${pack.topic}-practical-${number}`), className, subject: "Computing", term: "All Terms", topic: pack.topic, questionType: "Practical", difficulty: index < 2 ? "Moderate" : "Challenging", questionText, options: [], answer, marks: 20, createdBy: "Built-in NaCCA Computing bank", createdAt: "", source: "Built-in" };
  });
}
