...
function weeklyActivities(subject: string, className: string, week: string, strand: string, subStrand: string): DayRow[] {
  ...
  if (coreSubject) {
    rows.push(
      { dayDate: "Wednesday", starter: `Learners revisit the first lesson on ${topic} by explaining one important idea to a partner.`, main: `1. Teacher reviews progress from the first lesson and models a more challenging example.\n2. Learners complete an individual ${subject} task based on ${topic}.\n3. Learners compare answers or products in small groups and justify their choices.\n4. Teacher provides corrective feedback and supports learners who require remediation.\n5. Learners apply the improved understanding in a second task.`, reflection: `Learners summarise their new understanding of ${topic}. Teacher uses oral questions, observation and a short written exercise to assess progress.` },
      { dayDate: rows.some((row) => row.dayDate === "Thursday") ? "Friday" : "Thursday", starter: `Learners complete a short retrieval activity linking the week's ${subject} lessons on ${topic}.`, main: `1. Teacher presents an integrated task that draws on the knowledge and skills developed during the week.\n2. Learners work independently before comparing responses in pairs.\n3. Teacher reviews common strengths and misconceptions using selected learner responses.\n4. Learners improve their work and complete a short differentiated assessment task.\n5. Teacher records progress and identifies learners for enrichment or additional support.`, reflection: `Learners explain how their understanding of ${topic} has improved across the four lessons. Teacher reviews the assessment evidence and plans the next learning step.` },
    );
    const weekdayOrder = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"];
    rows.sort((left, right) => weekdayOrder.indexOf(left.dayDate) - weekdayOrder.indexOf(right.dayDate));
  }
  return rows;
}
